...
#
# Writes: dist/msx-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix msx \
  --exe-name Metal_Slug_X__Recompiled \
  --display-name "Metal Slug X  Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Metal Slug X disc" \
